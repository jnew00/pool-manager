...
import { analyzeRevengeGame, getRevengeGameSummary, REVENGE_FACTORS } from '../revenge-game'
...
describe('Revenge Game System', () => {
  ...
  describe('analyzeRevengeGame', () => {
    it('should return no revenge motivation when no previous meetings exist', async () => {
      ...
      const result = await analyzeRevengeGame(homeTeamId, awayTeamId, currentGameDate, currentSeason)
    ...
    it('should identify revenge game when home team lost previous meeting', async () => {
      ...
      expect(result.revengeMotivation).toBeGreaterThan(REVENGE_FACTORS.BASE_REVENGE_BONUS)
      expect(result.revengeMotivation).toBeLessThanOrEqual(REVENGE_FACTORS.MAX_REVENGE_BONUS)
    ...
    it('should identify revenge game when away team lost previous meeting', async () => {
      ...
      expect(result.revengeMotivation).toBeGreaterThan(REVENGE_FACTORS.BASE_REVENGE_BONUS) // Should include blowout bonus
    })
    ...
    it('should apply blowout bonus for losses of 14+ points', async () => {
      ...
      expect(result.revengeMotivation).toBeGreaterThan(REVENGE_FACTORS.BASE_REVENGE_BONUS + REVENGE_FACTORS.BLOWOUT_REVENGE_BONUS)
      expect(result.revengeMotivation).toBeLessThanOrEqual(REVENGE_FACTORS.MAX_REVENGE_BONUS)
    })
    ...
    it('should apply recent game multiplier for games within 6 weeks', async () => {
      ...
      const result = await analyzeRevengeGame(homeTeamId, awayTeamId, earlySeasonGameDate, currentSeason)
    ...
    it('should apply playoff multiplier for late season games', async () => {
      ...
      const result = await analyzeRevengeGame(homeTeamId, awayTeamId, lateSeasonGameDate, currentSeason)
    ...
    it('should cap revenge bonus at maximum allowed', async () => {
      ...
      const result = await analyzeRevengeGame(homeTeamId, awayTeamId, lateSeasonDate, currentSeason)
  ...
  describe('getRevengeGameSummary', () => {
    ...
    it('should generate proper summary for standard revenge game', () => {
      ...
      const summary = getRevengeGameSummary(result)
      expect(summary).toBe('Revenge game - home team lost by 7 points (+2.5 pts motivation)')
    })
  ...
  describe('REVENGE_FACTORS constants', () => {
    it('should have reasonable factor values', () => {
      ...
      expect(REVENGE_FACTORS.MAX_REVENGE_BONUS).toBeGreaterThan(REVENGE_FACTORS.BASE_REVENGE_BONUS)
    })
  })
})