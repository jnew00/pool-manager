...
export class NewsAnalysisService {
  ...
  constructor(config?: Partial<NewsAnalysisConfig>) {
    ...
    if (!this.config.apiKey) {
      switch (this.config.llmProvider) {
        ...
        case 'deepseek':
          ...
          this.config.apiEndpoint = this.config.apiEndpoint || 'https://api.deepseek.com/v1/chat/completions'
          this.config.model = this.config.model || 'deepseek-chat'
          break
        case 'anthropic':
          this.config.apiKey = process.env.ANTHROPIC_API_KEY || ''
          this.config.apiEndpoint = this.config.apiEndpoint || 'https://api.anthropic.com/v1/messages'
    ...
    if (!this.config.apiKey) {
      console.warn(`[News Analysis] ${this.config.llmProvider.toUpperCase()} API key not configured`)
    }
    ...
    console.log(`[News Analysis] Service initialized with provider: ${this.config.llmProvider}, enabled: ${this.config.enabled}, hasNewsApiKey: ${!!this.newsApiKey}, hasLLMKey: ${!!this.config.apiKey}`)
  }
  ...
  async analyzeGame(input: NewsAnalysisInput): Promise<NewsAnalysisResult> {
    try {
      console.log(`[News Analysis] Starting analysis for ${input.homeTeamName} vs ${input.awayTeamName} using ${this.config.llmProvider}`)
      
      // Check if service is enabled
      if (!this.config.enabled) {
        return this.createEmptyResult(input, 'News analysis disabled in configuration')
      }
      
      // Only perform analysis if confidence difference is within configured range
      if (Math.abs(input.confidenceDifference) < this.config.confidenceRangeMin! || 
          Math.abs(input.confidenceDifference) > this.config.confidenceRangeMax!) {
        return this.createEmptyResult(input, `Confidence difference (${Math.abs(input.confidenceDifference)}) outside range ${this.config.confidenceRangeMin}-${this.config.confidenceRangeMax}`)
      }

      // Fetch recent news for both teams
      console.log(`[News Analysis] Fetching news for ${input.homeTeamName} vs ${input.awayTeamName}`)
      ...
      console.log(`[News Analysis] Found ${homeNews.length} articles for ${input.homeTeamName}, ${awayNews.length} articles for ${input.awayTeamName}`)
      ...
      return this.createEmptyResult(input, `Analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
   ...
   */
  private async fetchTeamNews(teamName: string, gameDate: Date): Promise<NewsSource[]> {
    ...
    if (!this.newsApiKey) {
      console.warn('[News Analysis] News API key not configured, skipping news fetch')
      return []
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`[News Analysis] News API request failed: ${response.status}`)
        return []
      }
      ...
      return (data.articles || [])
        ...
        .sort((a: NewsSource, b: NewsSource) => b.relevanceScore - a.relevanceScore)
   ...
   */
  private async extractNewsFactors(input: NewsAnalysisInput, news: NewsSource[]): Promise<NewsFactor[]> {
    // Use mock data if configured
    if (this.config.useMockData) {
    ...
    if (!this.config.apiKey || news.length === 0) {
      if (!this.config.apiKey) {
        console.warn(`[News Analysis] ${this.config.llmProvider.toUpperCase()} API key not configured, skipping LLM analysis`)
      }
      return []
    }
  ...
  private async callOpenAIAPI(prompt: string): Promise<NewsFactor[]> {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert NFL analyst who identifies key factors from news that could impact game outcomes. Respond only with valid JSON.'
          },
          {
            role: 'user', 
            content: prompt
    ...
    if (!response.ok) {
      console.warn(`[News Analysis] OpenAI API request failed: ${response.status}`)
      return []
    }
  ...
  private async callDeepSeekAPI(prompt: string): Promise<NewsFactor[]> {
    const response = await fetch(this.config.apiEndpoint || 'https://api.deepseek.com/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${this.config.apiKey}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      console.warn(`[News Analysis] DeepSeek API request failed: ${response.status}`)
      return []
    }
  ...
  private async callAnthropicAPI(prompt: string): Promise<NewsFactor[]> {
    const response = await fetch(this.config.apiEndpoint || 'https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: this.config.maxTokens,
        temperature: this.config.temperature,
        system: 'You are an expert NFL analyst who identifies key factors from news that could impact game outcomes. Respond only with valid JSON.',
        messages: [
          {
            role: 'user',
            content: prompt
    ...
    if (!response.ok) {
      console.warn(`[News Analysis] Anthropic API request failed: ${response.status}`)
      return []
    }
  ...
  private async callCustomAPI(prompt: string): Promise<NewsFactor[]> {
    ...
    if (!response.ok) {
      console.warn(`[News Analysis] Custom API request failed: ${response.status}`)
      return []
    }
   ...
   */
  private buildAnalysisPrompt(input: NewsAnalysisInput, news: NewsSource[]): string {
    const newsText = news
      .map(n => `"${n.title}" (${n.source}, ${n.publishedAt.toLocaleDateString()})`)
      .join('\n')
...
`
   ...
   */
  private calculateRecommendation(input: NewsAnalysisInput, factors: NewsFactor[]): {
    confidence: number
    recommendedTeam?: 'HOME' | 'AWAY'
    summary: string
  } {
  ...
  private validateAndNormalizeFactors(factors: any[]): NewsFactor[] {
    const validTypes: NewsFactorType[] = [
      'INJURY_REPORT', 'WEATHER_CONCERN', 'TEAM_MORALE', 'COACHING_CHANGE',
      'SUSPENSION', 'TRADE_IMPACT', 'PLAYOFF_IMPLICATIONS', 'TRAVEL_FATIGUE',
      'ROSTER_DEPTH', 'RECENT_PERFORMANCE'
    ]
  ...
  private calculateRelevanceScore(article: any, teamName: string): number {
    ...
    const highImpactWords = ['injury', 'injured', 'suspended', 'out', 'questionable', 'doubtful']
    ...
    score += mediumImpactWords.filter(word => text.includes(word)).length * 0.1
   ...
   */
  private createEmptyResult(input: NewsAnalysisInput, reason: string): NewsAnalysisResult {
    return {
      gameId: input.gameId,
  ...
  private getMockNewsData(teamName: string, gameDate: Date): NewsSource[] {
    const mockNewsTemplates = [
      {
        titleTemplate: "{team} quarterback listed as questionable with ankle injury",
        type: 'INJURY_REPORT',
        impact: -2.5,
        confidence: 0.8,
        source: 'ESPN'
      ...
      {
        titleTemplate: "{team} star wide receiver cleared to play after hamstring concern",
        type: 'INJURY_REPORT', 
        impact: 1.5,
        confidence: 0.7,
        source: 'NFL.com'
      ...
      {
        titleTemplate: "{team} acquire veteran linebacker in trade deadline deal",
        type: 'TRADE_IMPACT',
        impact: 1.2,
        confidence: 0.6,
        source: 'NFL Network'
    ...
    return selectedTemplates.map((template, index) => ({
      ...
      publishedAt: new Date(gameDate.getTime() - (index + 1) * 24 * 60 * 60 * 1000), // Deterministic time
  ...
  private getMockFactorsFromNews(news: NewsSource[]): NewsFactor[] {
    const factorTypes: NewsFactorType[] = [
      'INJURY_REPORT', 'SUSPENSION', 'TEAM_MORALE', 'COACHING_CHANGE',
      'TRADE_IMPACT', 'WEATHER_CONCERN', 'TRAVEL_FATIGUE', 'ROSTER_DEPTH'
    ]
    ...
    return news.slice(0, 3).map((article, index) => {
      ...
      const factorType = factorTypes[Math.abs(titleHash + index) % factorTypes.length]
      ...
      if (factorType === 'INJURY_REPORT') {
        description = article.title.includes('questionable') ? 'Key player questionable with injury' : 'Player injury concern reported'
