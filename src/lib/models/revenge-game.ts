...
function calculateRevengeMotivation(
  ...
  const losingTeamId = mostRecentGame.winnerId === mostRecentGame.homeTeamId 
  ...
  const daysSinceLastMeeting = Math.floor(
    (currentGameDate.getTime() - mostRecentGame.gameDate.getTime()) / (1000 * 60 * 60 * 24)
  )
  
  if (daysSinceLastMeeting <= 42) { // 6 weeks
  ...
  const finalRevengeBonus = Math.min(revengeBonus, REVENGE_FACTORS.MAX_REVENGE_BONUS)
