...
const NFL_TEAM_LOCATIONS = {
  // AFC East
  'BUF': { city: 'Buffalo', lat: 42.8864, lon: -78.8784, timezone: 'America/New_York' },
  'MIA': { city: 'Miami', lat: 25.7617, lon: -80.1918, timezone: 'America/New_York' },
  'NE': { city: 'Boston', lat: 42.3601, lon: -71.0589, timezone: 'America/New_York' },
  'NYJ': { city: 'New York', lat: 40.7128, lon: -74.0060, timezone: 'America/New_York' },

  // AFC North  
  'BAL': { city: 'Baltimore', lat: 39.2904, lon: -76.6122, timezone: 'America/New_York' },
  'CIN': { city: 'Cincinnati', lat: 39.1031, lon: -84.5120, timezone: 'America/New_York' },
  'CLE': { city: 'Cleveland', lat: 41.4993, lon: -81.6944, timezone: 'America/New_York' },
  'PIT': { city: 'Pittsburgh', lat: 40.4406, lon: -79.9959, timezone: 'America/New_York' },

  // AFC South
  'HOU': { city: 'Houston', lat: 29.7604, lon: -95.3698, timezone: 'America/Chicago' },
  'IND': { city: 'Indianapolis', lat: 39.7684, lon: -86.1581, timezone: 'America/New_York' },
  'JAX': { city: 'Jacksonville', lat: 30.3322, lon: -81.6557, timezone: 'America/New_York' },
  'TEN': { city: 'Nashville', lat: 36.1627, lon: -86.7816, timezone: 'America/Chicago' },

  // AFC West
  'DEN': { city: 'Denver', lat: 39.7392, lon: -104.9903, timezone: 'America/Denver' },
  'KC': { city: 'Kansas City', lat: 39.0997, lon: -94.5786, timezone: 'America/Chicago' },
  'LV': { city: 'Las Vegas', lat: 36.1699, lon: -115.1398, timezone: 'America/Los_Angeles' },
  'LAC': { city: 'Los Angeles', lat: 34.0522, lon: -118.2437, timezone: 'America/Los_Angeles' },

  // NFC East
  'DAL': { city: 'Dallas', lat: 32.7767, lon: -96.7970, timezone: 'America/Chicago' },
  'NYG': { city: 'New York', lat: 40.7128, lon: -74.0060, timezone: 'America/New_York' },
  'PHI': { city: 'Philadelphia', lat: 39.9526, lon: -75.1652, timezone: 'America/New_York' },
  'WAS': { city: 'Washington', lat: 38.9072, lon: -77.0369, timezone: 'America/New_York' },

  // NFC North
  'CHI': { city: 'Chicago', lat: 41.8781, lon: -87.6298, timezone: 'America/Chicago' },
  'DET': { city: 'Detroit', lat: 42.3314, lon: -83.0458, timezone: 'America/New_York' },
  'GB': { city: 'Green Bay', lat: 44.5133, lon: -88.0133, timezone: 'America/Chicago' },
  'MIN': { city: 'Minneapolis', lat: 44.9778, lon: -93.2650, timezone: 'America/Chicago' },

  // NFC South
  'ATL': { city: 'Atlanta', lat: 33.7490, lon: -84.3880, timezone: 'America/New_York' },
  'CAR': { city: 'Charlotte', lat: 35.2271, lon: -80.8431, timezone: 'America/New_York' },
  'NO': { city: 'New Orleans', lat: 29.9511, lon: -90.0715, timezone: 'America/Chicago' },
  'TB': { city: 'Tampa', lat: 27.9506, lon: -82.4572, timezone: 'America/New_York' },

  // NFC West
  'ARI': { city: 'Phoenix', lat: 33.4484, lon: -112.0740, timezone: 'America/Phoenix' },
  'LAR': { city: 'Los Angeles', lat: 34.0522, lon: -118.2437, timezone: 'America/Los_Angeles' },
  'SF': { city: 'San Francisco', lat: 37.7749, lon: -122.4194, timezone: 'America/Los_Angeles' },
  'SEA': { city: 'Seattle', lat: 47.6062, lon: -122.3321, timezone: 'America/Los_Angeles' }
} as const
 ...
 */
function calculateTimeZoneDifference(fromTimezone: string, toTimezone: string): number {
  const fromOffset = TIMEZONE_OFFSETS[fromTimezone as keyof typeof TIMEZONE_OFFSETS] || 0
  const toOffset = TIMEZONE_OFFSETS[toTimezone as keyof typeof TIMEZONE_OFFSETS] || 0
...
function isShortWeek(kickoffTime: Date, lastGameDate?: Date): boolean {
  ...
  const daysDiff = (kickoffTime.getTime() - lastGameDate.getTime()) / (1000 * 60 * 60 * 24)
...
export function calculateTravelSchedulingFactors(
  ...
  const awayTeamLocation = NFL_TEAM_LOCATIONS[input.awayTeamId as keyof typeof NFL_TEAM_LOCATIONS]
  const homeTeamLocation = NFL_TEAM_LOCATIONS[input.homeTeamId as keyof typeof NFL_TEAM_LOCATIONS]
  ...
  // Create description
  const factors = []
  if (travelDistance > 1500) factors.push(`${Math.round(travelDistance)} mile trip`)
  if (Math.abs(timeZoneChange) >= 2) factors.push(`${Math.abs(timeZoneChange)} hour time change`)
...
export function calculateTravelDisadvantage(
  ...
  homeTeamId: string,
  kickoffTime: Date
): { hasDisadvantage: boolean; severity: 'MINOR' | 'MODERATE' | 'SEVERE'; factors: string[] } {
  ...
  const severity = factors.advantage >= 2.5 ? 'SEVERE' :
                  factors.advantage >= 1.5 ? 'MODERATE' : 'MINOR'

  const factorList = []
  if (factors.travelDistance > 1500) factorList.push('long distance travel')
  if (Math.abs(factors.timeZoneChange) >= 2) factorList.push('time zone disruption')
