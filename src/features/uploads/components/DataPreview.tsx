      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Review your data before importing. Click on rows to see validation details.
          </p>
        </div>
        ...
        <div className="flex space-x-2">
          ...
          <button
            ...
            className={`px-3 py-1 text-sm rounded ${
              filter === 'valid' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'
            }`}
          >
            Show Valid Only
          </button>
          <button
            ...
            className={`px-3 py-1 text-sm rounded ${
              filter === 'invalid' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700'
            }`}
          >
            Show Invalid Only
          </button>
          ...
          <table className="w-full border border-gray-200 rounded-lg">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Away</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Home</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Spread</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Total</th>
              </tr>
            </thead>
            <tbody>
              {paginatedData.map(({ game, validation, originalIndex }, displayIndex) => (
                <React.Fragment key={originalIndex}>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm font-medium">{game.away_team}</td>
                    <td className="px-4 py-3 text-sm font-medium">{game.home_team}</td>
                    <td className="px-4 py-3 text-sm">{game.spread || '-'}</td>
                    <td className="px-4 py-3 text-sm">{game.total || '-'}</td>
                  </tr>
                  
                  {/* Error Details Row */}
                  {selectedRowIndex === originalIndex && !validation.valid && (
                    <tr>
                      <td colSpan={7} className="px-4 py-3 bg-red-100 border-t border-red-200">
                        <div className="space-y-1">
                          ...
                          {validation.warnings.length > 0 && (
                            <>
                              <div className="font-medium text-yellow-800 mt-2">Warnings:</div>
                              {validation.warnings.map((warning, warningIndex) => (
                                <div key={warningIndex} className="text-sm text-yellow-700">
                                  • {warning}
                                </div>
                              ))}
                            </>
